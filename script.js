document.addEventListener('DOMContentLoaded', () => {
  ...
  // Function to display the shortened link
  function displayShortenedLink(shortURL, originalURL) {
    ...
    copyButton.addEventListener('click', () => {
      navigator.clipboard.writeText(shortURL).then(() => {
          ...
          copyButton.classList.add('clicked'); // Add the class
          setTimeout(() => {
              copyButton.textContent = 'Copy';
              copyButton.classList.remove('clicked'); // Remove the class after 5 seconds
          }, 5000);
      });
  });
  ...
  // URL shortening button click event
